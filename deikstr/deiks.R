...
#поиск элемента с минимальной меткой
minn <- function(metki,were){
  min <- Inf
  x <- 1 #элемент с минимальной меткой
  if(is.null(were)){ #если ветор пустой
    were[1] <- -1
  }
  
  for(i in 1:length(metki)){
    if(metki[i] < min && all(i!=were)){ #ищем минимальную метку для вершины в которой еще не были
      min <- metki[i]
      x <- i
    }
  }
...
format_vertex <- paste("??????? -", seq(1,n),"\n", "???? =", metki) #массив с подписями для вершин
